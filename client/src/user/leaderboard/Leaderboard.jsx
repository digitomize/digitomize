...
export default function Leaderboard() {
  ...
  const handleChange = (event) => {
    ...
    setSearchParams({ platform : event.target.value
    })
    fetchLbData()
  };
  ...
  async function fetchLbData() {
    try {
      ...
      res = await fetch(`https://www.v2api.digitomize.com/user/leaderboard?platform=${searchParams.platform}?page=${page}`)
      else
      res= await fetch("https://www.v2api.digitomize.com/user/leaderboard")
  ...
  const handlePageChange = (event, value) => {
    ...
    navigate(`${location.pathname}?page=${value}?platform=${searchParams.get(platform)}`);
    else
  ...
  return (
    <>
      ...
      <div className="phone:w-4/6 w-11/12 mx-auto">
      <Box className={"w-[170px] flex ml-auto"} >
      <FormControl className={"bg-[#474748] rounded-[8px] w-full text-[16px]"} variant="outlined">
        <InputLabel id="demo-simple-select-label" className={"text-white text-[16px]"} >select platform</InputLabel>
        <Select
