    ...
    <div className={`pt-${pt}`}>
      {user?.username && user?.name ? (
        <Link to={"/u/" + user?.username}>
          <div className="noCursor relative flex justify-center flex-col items-center">
            <div className="noCursor flex z-[10] flex-col items-center justify-center sm:translate-y-7 translate-y-4">
              {/* Badge */}
              <svg
                ...
                viewBox="0 0 66 50"
                fill="none"
                className="noCursor sm:w-[66px] sm:h-[50px]  w-[29px] h-[24px]"
              >
                <path
              ...
              {/* Profile Picture */}
              <img
                className="noCursor rounded-full sm:w-[108px] sm:h-[108px] w-[50px] h-[50px] mask mask-circle"
            ...
            {color === "#FFD700" ? (
              <div className="noCursor bg-[#252525]  rounded-[12px] text-center gap-x-[8px] max-sm:h-[94px] sm:h-[132px] sm:w-[110px] max-sm:w-[55px] flex flex-col items-center justify-center font-semibold tracking-[0.42px] sm:text-[14px] text-[10px]">
                <h2 className="noCursor text-[#fff]">{user?.name}</h2>
                <h2 style={{ color: color }}>{Math.floor(rating)}</h2>
                <h2
                  style={{ color: color }}
                  className="noCursor absolute max-sm:bottom-2 bottom-3"
                >
                  #1
                </h2>
              </div>
            ) : color === "#C0C0C0" ? (
              <div className="noCursor bg-[#252525]  rounded-[12px] text-center gap-x-[8px] max-sm:h-[82px] sm:h-[117px] sm:w-[110px]  max-sm:w-[55px] flex flex-col items-center justify-center font-semibold tracking-[0.42px] sm:text-[14px] text-[10px]">
                <h2 className="noCursor text-[#fff]">{user?.name}</h2>
                <h2 style={{ color: color }}>{Math.floor(rating)}</h2>
                <h2
                  style={{ color: color }}
              ...
              <div className="noCursor bg-[#252525]  rounded-[12px] text-center gap-x-[8px] max-sm:h-[75px] sm:h-[107px] sm:w-[110px]  max-sm:w-[55px] flex flex-col items-center justify-center font-semibold tracking-[0.42px] sm:text-[14px] text-[10px]">
                <h2 className="noCursor text-[#fff]">{user?.name}</h2>
                <h2 style={{ color: color }}>{Math.floor(rating)}</h2>
                <h2
                  style={{ color: color }}
        ...
        <>
          <div className="noCursor relative flex justify-center flex-col items-center">
            <div className="noCursor flex z-[10] flex-col items-center justify-center translate-y-10">
              {/* Badge */}
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="66"
          ...
          <div className="noCursor bg-[#252525] rounded-[12px] text-center gap-x-[8px] sm:h-[120px] max-sm:h-[90px] sm:w-[110px] max-sm:w-[55px] flex flex-col items-center justify-center font-semibold tracking-[0.42px] text-[14px]">
            <Skeleton
              variant="rounded"
              width={"full"}
