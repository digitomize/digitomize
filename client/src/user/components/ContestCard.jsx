...
function ContestCard({ data }) {
  ...
  return (
    <Link to={`/contests/${vanity}`}>
      <div className="noCursor flex flex-1 px-3 py-2 flex-row   space-x-2 border border-solid border-[#EBEBEB] bg-eerie-black-2 rounded-[2px] min-w-[200px]">

        <img
          src={hostToSVGMap[host]}
          alt={host}
          className="noCursor w-[45px] h-[45px] my-auto"
        />

        <div className="noCursor flex flex-col">
          <p

            className="noCursor my-auto text-[10px]"

          >
            {`${startMonth} ${startDate}, ${startYear} at ${startTime}`}
          </p>
          <div className="noCursor my-auto">
            <h2 className="noCursor text-[12px] text-left ">{name}</h2>
          </div>
        </div>
      </div>
    </Link>
