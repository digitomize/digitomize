      ...
      <div className="hidden w-56 md:flex flex-col border-r border-solid border-jet bg-cardsColor h-screen">
        <div className="">
          <img
            src={logo_white_full}
            alt="logo"
            className="w-9/12 mx-auto m-4"
          />
        </div>

        <Link
          to={"/u/dashboard"}
          className={`capitalize flex flex-row items-center gap-2 mx-auto my-2`}
        >
          <FaArrowLeft />
          Dashboard
        </Link>
        ...
        <div className="w-9/12 mx-auto">
          <div>
            <p className="text-sm text-slate-500 font-thin text-left">
              Account
            </p>
            <div className="text-sm flex flex-col items-start w-full items-stretch">
              {accountLinks.map((option, index) => (
                <Link to={option.path} key={index}>
                  <button
                    className={`justify-start flex w-full btn btn-ghost ${
                  ...
                  >
                    ...
                    <p className="capitalize font-[500] text-sm text-[#EBEBEB]">
                      {option.title}
                    </p>
                  </button>
                </Link>
            ...
            {navLinks.map((option, index) => (
              <Link
                ...
                className={`capitalize ${
                  selectedOption === option.title ? "text-button-primary" : ""
                }`}
                onClick={() => {
                  handleOptionClick(option.title);
                }}
