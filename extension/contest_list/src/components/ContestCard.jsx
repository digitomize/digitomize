...
function ContestCard({ contest }) {
  ...
  return (
    <div className="flex flex-row gap-2 border rounded border-jet bg-dashboardDarkerColor mt-2 p-2 w-full">
      <div>
        {platformLogo && (
          <img src={platformLogo} alt={contest.name} className="w-8" />
        )}
      </div>
      <div className="flex flex-col text-sm w-11/12">
        ...
        <div className="flex flex-row items-center gap-1">
          <FaClock />
          <p>
            Duration: {Math.floor(contest.duration / 60)}hr{" "}
            {contest.duration % 60 ? `${contest.duration % 60}min` : ""}
          </p>
        </div>
        <div className="flex flex-row justify-between items-center w-full">
          <p>
            Starts at:{" "}
            <a href={timeAndDateURL} target="_blank">
              {" "}
