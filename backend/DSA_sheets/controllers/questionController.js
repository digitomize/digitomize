...
export const createQuestions = async (req, res) => {
  try {
    ...
    const existingQuestions = await QuestionModel.find({ q_id: { $in: questionsData.map(q => q.q_id) } });
    ...
    const newQuestions = questionsData.map(question => new QuestionModel(question));
