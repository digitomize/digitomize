...
function createEmbeds(titlePrefix, username, image, data) {
    ...
    currentEmbed.addFields(
        ...
        { name: 'dateOfBirth', value: safeString(data?.dateOfBirth?.data), inline: true },
        { name: 'phoneNumber', value: safeString(data?.phoneNumber?.data), inline: true },
        { name: 'skills', value: safeString(data?.skills?.join(', ')), inline: true },
        { name: 'github', value: safeString(data?.github?.data), inline: true },
        { name: 'codechef', value: safeString(data?.codechef ? formatCodechef(data.codechef) : 'Not Available'), inline: true },
        { name: 'leetcode', value: safeString(data?.leetcode ? formatLeetCode(data.leetcode) : 'Not Available'), inline: true },
        { name: 'codeforces', value: safeString(data?.codeforces ? formatCodeforces(data.codeforces) : 'Not Available'), inline: true },
        { name: 'digitomize_rating', value: safeString(data?.digitomize_rating), inline: true }
    );
...
function formatCodechef(codechefData) {
    return `${codechefData?.username} - Rating: ${codechefData?.rating ?? 'Not Available'} - Badge: ${codechefData?.badge ?? 'Not Available'}`;
}

function formatLeetCode(leetcodeData) {
    return `${leetcodeData?.username} - Rating: ${leetcodeData?.rating ?? 'Not Available'} - Badge: ${leetcodeData?.badge ?? 'Not Available'}`;
}

function formatCodeforces(codeforcesData) {
    return `${codeforcesData?.username} - Rating: ${codeforcesData?.rating ?? 'Not Available'} - Badge: ${codeforcesData?.badge ?? 'Not Available'}`;
}
