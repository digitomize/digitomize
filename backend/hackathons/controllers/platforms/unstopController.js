      ...
      return new Promise((resolve, reject) => {
        response.on("end", async function() {
          try {
            ...
            const formattedHackathons = hackathonsList.data.data.map(
              (hackathon) => ({
                host: "unstop",
                ...
                registerationStartTimeUnix: Math.floor(
                  new Date(hackathon.regnRequirements.start_regn_dt).getTime() /
                    1000,
                ),
                registerationEndTimeUnix: Math.floor(
                  new Date(hackathon.regnRequirements.end_regn_dt).getTime() /
                    1000,
                ),
            ...
            if (page === 1) {
              resolve([
                formattedHackathons,
                hackathonsList.data.total,
                hackathonsList.data.per_page,
              ]);
            } else {
