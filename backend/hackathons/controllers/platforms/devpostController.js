...
async function fetchPageWise(url, page) {
  ...
  const hackathonsPromise = promise
      ...
      return new Promise((resolve, reject) => {
        response.on("end", async function() {
          try {
            ...
            const formattedHackathons = hackathonsList.hackathons.map(
              async(hackathon) => {
                const hackathonInfo = {};
                ...
                try {
                  ...
                  const starts_at_iso = $(
                    `${htmlElement} td:nth-child(2)`,
                  ).attr("data-iso-date");
                  const ends_at_iso = $(`${htmlElement} td:nth-child(3)`).attr(
                    "data-iso-date",
                  );

                  const startTimeUnix = Math.floor(
                    new Date(starts_at_iso).getTime() / 1000,
                  );
                  const endTimeUnix = Math.floor(
                    new Date(ends_at_iso).getTime() / 1000,
                  );

                  hackathonInfo.hackathonStartTimeUnix = startTimeUnix;
                  hackathonInfo.duration = Math.floor(
                    (endTimeUnix - startTimeUnix) / 60,
                  );
            ...
            Promise.all(formattedHackathons).then((data) => {
              if (page === 1) {
                resolve([
                  data,
                  hackathonsList.meta.total_count,
                  hackathonsList.meta.per_page,
                ]);
              } else {
                resolve(data);
              }
  ...
  return new Promise((resolve) => {
    ...
    fetchPageWise(`${url}&page=1`, 1)
      .then((firstPageHackathons) => {
        ...
        const totalPages = Math.ceil(totalHackathons / per_page);

        const fetchPromises = [];
        for (let pageNumber = 2; pageNumber <= totalPages; ++pageNumber) {
          fetchPromises.push(
            fetchPageWise(`${url}&page=${pageNumber}`, pageNumber),
          );
        }
